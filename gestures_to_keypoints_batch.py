...
parser.add_argument('--path_to_csv', default="./keypoints/csvs", type=str, help='Path to save the csv containing keypoints.')
parser.add_argument('--path_to_keypoint_imgs', default="./keypoints/keypoints_imgs", type=str, help='Path to save the images with keypoints marked.')

args = parser.parse_args()

# checking if the path to save keypoints exists or not
if not os.path.exists(args.path_to_csv):
        print(f"~~~ The directory '{args.path_to_csv}' does not exist. ~~~~")
        print("===> Creating the directory - {}".format(args.path_to_csv))
        os.makedirs(args.path_to_csv)
        print("===> Directory created")

if not os.path.exists(args.path_to_keypoint_imgs):
        print(f"~~~ The directory '{args.path_to_keypoint_imgs}' does not exist. ~~~~")
        print("===> Creating the directory - {}".format(args.path_to_keypoint_imgs))
        os.makedirs(args.path_to_keypoint_imgs)
        print("===> Directory created")
...
for i, video_file in enumerate(directory_contents):
    print("\t>>>>>> Progress: {}/{} >>>>>>".format(i+1,len(directory_contents)))
    print("\n***** PROCESSING VIDEO FILE: ", video_file, " *****")
    print("===> Loading the video")
    video_loc = os.path.join(args.input,video_file)
    ...
    print("===> Saving the csv file containing keypoints at ", "{}/keypoints_{}.csv".format(args.path_to_csv, "_".join(video_loc.split('/')[-1].split('.'))))
    df.to_csv("{}/keypoints_{}.csv".format(args.path_to_csv, "_".join(video_loc.split('/')[-1].split('.'))),index=False)
